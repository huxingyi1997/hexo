---
title:  Typescript速成自学
date:  2021-04-07 14:30:00
categories: 
- web前端
tags:
- 编程语言
...
TypeScript 是 JavaScript 语言的超集，为 JavaScript 添加了可选择的类型标注，大大增强了代码的可读性和可维护性。同时，它提供最新和不断发展的 JavaScript 特性，能让我们建立更丰富的组件。
...
![img](https://cdn.jsdelivr.net/gh/huxingyi1997/my_img/img/20210418202745.png)
...
我们使用了 `.ts` 扩展名，但是这段代码仅仅是 原生JavaScript 而已。
...
最后，让我们使用类来改写这个例子。 TypeScript 支持 JavaScript 的ES6新特性，比如支持基于类的面向对象编程。
...
TypeScript和 JavaScript 一样，可以使用双引号（`"`）或单引号（`'`）表示字符串。
...
通过类型断言这种方式可以告诉编译器，“相信我，我知道自己在干什么”。 类型断言好比其它语言里的类型转换，但是不进行特殊的数据检查和解构。 它没有运行时的影响，只是在编译阶段起作用。 TypeScript 会假设程序员已经进行了必须的检查。
...
两种形式是等价的。 然而，当你在 TypeScript 里使用 JSX 时，只有 `as` 语法断言是被允许的。
...
还有一点值得提的是，类型检查器不会去检查属性的顺序，只要相应的属性存在并且类型也是对的就可以（就当对象理解好了）。
...
// error TS2345: Argument of type '{ colour: string; width: number; }' is not assignable to parameter of type 'SquareConfig'.
...
console.log('card: ' + pickedCard.card + ' of ' + pickedCard.suit); // card: 2 of hearts
```
...
上一节，创建的 `identity` 通用函数，可以适用于不同的类型。 在这节种研究一下函数本身的类型，以及如何创建泛型接口。
...
console.log(stringNumeric.add(stringNumeric.zeroValue, 'test')); // script.ts(1,25): error TS2304: Cannot find name 'GenericNumber'.
```
...
TypeScript 具有两种特殊的类型，`null` 和 `undefined`，它们分别具有值 `null` 和 `undefined`。我们在基础类型一节里已经做过简要说明。 默认情况下，类型检查器认为 `null` 与 `undefined` 可以赋值给任何类型。 `null` 与 `undefined` 是所有其它类型的一个有效值。 这也意味着，你阻止不了将它们赋值给其它类型，就算是你想要阻止这种情况也不行。`null`的发明者，Tony Hoare，称它为价值亿万美金的错误 (opens new window)。
